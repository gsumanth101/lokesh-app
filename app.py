...
def recommend_crop(soil_type, rainfall_mm, temperature_c, location):
    ...
    try:
        ...
        if "quota" in str(e).lower() or "rate limit" in str(e).lower():
            st.warning("Rate limit exceeded. Please wait before making another request.")
        else:
    ...
    try:
        ...
        for i in range(7):
            ...
            day_forecast = {
                ...
                'date_formatted': forecast_date.strftime('%m/%d'),
                ...
                'description': 'Partly Cloudy' if base_rainfall < 5 else 'Rainy' if base_rainfall > 15 else 'Cloudy'
            }
            
            forecast_data.append(day_forecast)
        
        # Return in the expected format with success indicator
        return {
            'success': True,
...
def get_nasa_weather_time_series(lat, lon):
    ...
    try:
        ...
        st.info(f"Fetching 14-day weather time series from NASA Power API...")
        ...
        if weather_time_series:
            ...
            if weather_analysis:
                ...
                if weather_analysis['critical_conditions']:
                    st.warning(f"    Critical conditions: {', '.join(weather_analysis['critical_conditions'])}")
        ...
        try:
            ...
            if updated_message.error_code:
                ...
                if updated_message.error_code == 21614:
                    ...
                    st.error("  Go to Twilio Console â†’ Phone Numbers â†’ Verified Caller IDs")
...
def display_pesticide_recommendations(recommendations):
    ...
    try:
        ...
        create_section_header("Environmental Analysis", " ")
        ...
        metrics = [
            ("Temperature", f"{env_factors['temperature']:.1f}°C", " ", "#ff6b6b"),
            ("Humidity", f"{env_factors['humidity']:.1f}%", " ", "#4ecdc4"),
            ("Rainfall", f"{env_factors['rainfall']:.1f}mm", " ", "#45b7d1"),
        ...
        if chemical_recs:
            ...
            <div class="modern-card pesticide-card fade-in-scale">
                <h3 style="color: var(--chemical-color); margin-bottom: 1.5rem; display: flex; align-items: center;">
                    <span class="icon-wrapper" style="background: var(--chemical-color); margin-right: 1rem; width: 40px; height: 40px; font-size: 1.2rem;"> </span>
                    Chemical Solutions
                </h3>
            """, unsafe_allow_html=True)
        ...
        with st.spinner("  Fetching location coordinates and weather data..."):
            ...
            try:
                ...
                with col2:
                    # Crop emoji mapping
                    crop_emoji = {
                        'rice': '', 'maize': '', 'wheat': '', 'cotton': 'ðŸŒ¿',
                        'banana': '', 'mango': '', 'grapes': 'ðŸ‡', 'apple': 'ðŸŽ',
                        'orange': '', 'papaya': '', 'coconut': 'ðŸ¥¥', 'coffee': 'â˜•',
                        'chickpea': '', 'kidneybeans': 'ðŸ«˜', 'blackgram': 'ðŸ«˜',
                        'lentil': '', 'mungbean': '', 'mothbeans': 'ðŸ«˜',
                        'pigeonpeas': '', 'jute': '', 'pomegranate': 'ðŸ‡',
                        'watermelon': '', 'muskmelon': 'ˆ'
                    }
...
def show_admin_dashboard():
    ...
    with col1:
        ...
        <div class="metric-card">
            <h3 style="color: #2E7D32; margin: 0;">ðŸ‘¨â€  {stats['total_farmers']}</h3>
                    ...
                    for change in recent_changes[:10]:  # Show top 10 recent changes
                        if change.get('old_price') and change.get('new_price'):
                            ...
                            if old_price > 0:
                                ...
                                st.write(f"{change_icon} **{change['crop_name'].title()}**:  {old_price:.0f} â†’  {new_price:.0f} ({change_percent:.1f}%) by {change['updated_by_name']} ({change['updated_by_role']})")
                else:
...
def show_admin_weekend_farming_management():
    ...
    if recent_posts:
        ...
        for post in recent_posts:
            ...
            visibility_icon = "  " if is_hidden else ""
...
def show_lstm_disease_prediction():
    ...
    try:
        ...
        if not location or location == 'Unknown':
            st.warning("    Please complete crop recommendation first to get accurate disease predictions based on your location and weather data.")
        ...
        with st.spinner("     Fetching 7-day weather forecast for disease analysis..."):
            weather_forecast = get_7day_weather_forecast(lat, lon)
            
        if not weather_forecast or not weather_forecast.get('success'):
        ...
        if i < len(forecast_data):
            ...
            with tab:
                ...
                if daily_risk_score > 0.6:
                    ...
                    st.warning(f"    Moderate Risk Day - Score: {daily_risk_score:.2f}")
                else:
    ...
    try:
        ...
        if prices:
            ...
            if volatility_ratio > 20:
                insights.append(f"    High price volatility detected ({volatility_ratio:.1f}%) - market conditions vary significantly")
        ...
        <div style="text-align: center; color: #7F8C8D; font-size: 14px; padding: 20px;">
            ...
            🔄 <strong>Cache:</strong> 1 hour |     <strong>Disclaimer:</strong> Prices are indicative and may vary
        </div>
        """, unsafe_allow_html=True)
...
def show_agent_crop_selling_module():
    ...
    with st.form("agent_crop_listing_form"):
        ...
        st.markdown("#### ðŸ‘¨â€  Farmer Information")
        col1, col2 = st.columns(2)
        ...
        for listing in listings:
            with st.expander(f"{listing['crop_name'].title()} - {listing['quantity']} kg -  {listing['expected_price']}/kg - Farmer: {listing['farmer_name']}"):
                ...
                with col2:
                    ...
                    st.write(f"ðŸ‘¨â€  **Name:** {listing['farmer_name']}")
                    st.write(f" **Phone:** {listing['farmer_phone']}")
                    st.write(f"   **Total Value:**  {listing['quantity'] * listing['expected_price']:,.2f}")
    else:
...
def show_agent_market_management():
    ...
    tab1, tab2, tab3, tab4 = st.tabs(["ðŸ“ˆ Current Prices", "ðŸ“ Update Prices", "  Price History", " Notifications"])
    ...
    with tab2:
        ...
        with st.form("market_price_update_form"):
            ...
            if st.form_submit_button("ðŸ’¾ Update Market Price"):
                ...
                if success:
                    st.success(f"âœ… Market price for {selected_crop.title()} updated to  {new_price}/quintal (Trend: {trend})")
        ...
        if price_logs:
            ...
            if recent_changes:
                ...
                for change in recent_changes[:5]:  # Show top 5 recent changes
                    change_direction = "ðŸ“ˆ" if change['price_change_percent'] > 0 else "ðŸ“‰" if change['price_change_percent'] < 0 else " "
                    st.write(f"{change_direction} **{change['crop_name'].title()}**:  {change['old_price']:.0f} â†’  {change['new_price']:.0f} ({change['price_change_percent']:.1f}%) by {change['updated_by_name']}")
            else:
...
def show_agriculture_news():
    ...
    try:
            ...
            with st.spinner("Fetching latest agriculture news from India..."):
                ...
                if news_data and (news_data.get('status') == 'success' or news_data.get('results')):
                    ...
                    if articles:
                        ...
                        st.warning(" No agriculture news articles found at the moment.")
        ...
        with st.expander(" About Agriculture News"):
            st.markdown("""
            **   News Sources:**
    ...
    if 'last_recommended_crop' in st.session_state and 'last_location' in st.session_state:
        ...
        if weather_forecast and weather_forecast.get('success'):
            ...
            if avg_temp < 15 or avg_temp > 38: # Temperature check
                is_suitable = False
                reasons.append(f" Average temperature ({avg_temp:.1f}°C) is outside the optimal range for {crop_name}.")
            
            if total_rainfall > 150: # Excessive rain
                is_suitable = False
                reasons.append(f"High rainfall ({total_rainfall:.1f} mm) may cause waterlogging.")

            if high_wind_days > 3:
                is_suitable = False
                reasons.append(f"Frequent high winds may damage the crop.")
            ...
            if is_suitable:
                ...
                <div style="background-color: #27ae60; padding: 25px; border-radius: 15px; text-align: center; color: white;">
                    ...
                    <p style="font-size: 1.1rem;">The 7-day forecast is favorable for cultivating <strong>{}</strong>.</p>
                </div>
                """, unsafe_allow_html=True)
            else:
                st.markdown("""
            ...
            forecast_tab1, forecast_tab2, forecast_tab3, forecast_tab4 = st.tabs([" Overview", " Temperature", " Rainfall", " Wind Speed"])
            ...
            with forecast_tab2:
                ...
                st.info(f" Temperature Range: {min_temp:.1f}°C to {max_temp:.1f}°C")
            
            with forecast_tab3:
                st.markdown("#####  Rainfall Forecast (mm)")
                st.bar_chart(chart_df['Rainfall'], height=400)
                
                # Rainfall insights
                total_rainfall = chart_df['Rainfall'].sum()
                if total_rainfall > 50:
                    st.warning(f" Heavy rainfall expected: {total_rainfall:.1f}mm total")
                elif total_rainfall > 20:
                    st.info(f" Moderate rainfall expected: {total_rainfall:.1f}mm total")
                else:
                    st.success(f" Light rainfall expected: {total_rainfall:.1f}mm total")
            
            with forecast_tab4:
                st.markdown("#####  Wind Speed Forecast (m/s)")
                ...
                if max_wind > 15:
                    ...
                    st.info(f" Moderate winds expected: up to {max_wind:.1f}m/s")
                else:
        ...
        with st.form("feedback_form"):
            ...
            if submit_feedback:
                ...
                if not buyer_has_feedback:
                    ...
                    st.warning(f" You have already provided feedback for {selected_farmer_data['name']}.")
...
def create_modern_disease_card(disease_info, risk_level, prevention_tips):
    ...
    <div class="modern-card disease-prediction-card slide-in-up">
        ...
        <div style="margin-top: 1.5rem; padding: 1rem; background: rgba(211, 47, 47, 0.1); 
            ...
            <h4 style="margin: 0 0 1rem 0; color: {risk_color};"> Prevention & Management Tips</h4>
    """, unsafe_allow_html=True)
...
def create_modern_weather_dashboard(weather_data):
    ...
    weather_metrics = [
        ...
        ("", "Rainfall", f"{weather_data.get('daily_precipitation', 'N/A')} mm/day"),
        ("", "Wind Speed", f"{weather_data.get('wind_speed', 'N/A')} m/s"),
        ("", "Pressure", f"{weather_data.get('pressure', 'N/A')} hPa"),
        ("", "Solar Radiation", f"{weather_data.get('solar_radiation', 'N/A')} W/m²")
    ]
...
def show_weekend_farming_main(user_id, db_manager):
    ...
    if user_location is None:
        st.warning(" Your location is not set in your profile. Distances cannot be calculated. Please update your profile with your address.")
    ...
    if user_location and farms:
        with st.spinner(f" Calculating distances to {len(farms)} farms..."):
            ...
            for i, farm in enumerate(farms):
                ...
                if farm_location:
                    ...
                    if distance_info['success']:
                        ...
                        st.warning(f" Could not calculate exact distance to {farm['farmer_name']}: {distance_info.get('error', 'Unknown error')}")
                else:
            ...
            with st.expander(f"{indicator} {fav_icon} **{farm['farmer_name']}** - {farm['location']}", expanded=False):
                ...
                with col1:
                    ...
                    if duration_display and duration_display != 'Unknown':
                        st.markdown(f"** Travel Time:** {duration_display}")
                ...
                with col1:
                    if farm['farmer_id'] in fav_farm_ids:
                        if st.button(f" Remove from Favorites", key=f"unfav_{farm['id']}"):
...
def show_weekend_farming_profile(user_id, db_manager):
    ...
    with st.form("location_update_form"):
        ...
        col1, col2 = st.columns(2)
        with col1:
            test_location = st.form_submit_button(" Test Location")
        with col2:
