...
st.set_page_config(
    page_title="ðŸŒ¾ Smart Farming Assistant - AI-Powered Agriculture",
    page_icon="ðŸŒ¾",
    layout="wide",
...
def get_nasa_weather(lat, lon, date=None):
    ...
    try:
        ...
        st.info(f"ðŸ›°ï¸ Fetching weather data from NASA Power API...")
        st.info(f"ðŸ“ Coordinates: ({lat:.4f}, {lon:.4f})")
        st.info(f"ðŸ“… Date: {date}")
        ...
        if response.status_code == 200:
            ...
            st.success(f"âœ… Successfully retrieved weather data with {len(all_params)} parameters!")
            return data
        
        elif response.status_code == 422:
            st.warning("âš ï¸ Some parameters not supported. Trying with essential parameters only...")
            ...
            if response.status_code == 200:
                ...
                st.success(f"âœ… Retrieved weather data with {len(essential_params)} essential parameters!")
                return data
            else:
...
def display_pesticide_and_soil_recommendations(crop_name, ph, organic_matter, N, P, K):
    ...
    with col1:
        st.markdown("### ðŸ› Pesticide Recommendations")
        ...
        st.info("â€¢ Apply during early morning or evening")
        st.info("â€¢ Wear protective equipment (gloves, mask)")
        st.info("â€¢ Avoid application before rain")
        st.info("â€¢ Follow label instructions carefully")
    
    with col2:
        st.markdown("### ðŸŒ± Soil Health Recommendations")
        
        # Soil nutrient analysis
        st.markdown("**ðŸ“Š Current Soil Status:**")
...
# Display market price for recommended crop
                ...
                st.subheader("ðŸ¦  Enhanced Disease Prediction & Management")
                ...
                if disease_result and disease_result['disease'] != 'Healthy':
                    ...
                    if risk_level == 'High':
                        st.error(f"ðŸš¨ **{disease_name}** - {risk_level} Risk")
                        risk_color = "#ff4b4b"
                    elif risk_level == 'Medium':
                        st.warning(f"âš ï¸ **{disease_name}** - {risk_level} Risk")
                        risk_color = "#ff8c00"
                    else:
                        st.info(f"â„¹ï¸ **{disease_name}** - {risk_level} Risk")
                        risk_color = "#0066cc"
                    ...
                    with col1:
                        st.markdown("**ðŸŽ¯ Risk Factors:**")
                        for factor in disease_result['risk_factors']:
                            st.write(f"â€¢ {factor}")
                        
                        # Display timing information if available
                        if 'timing' in disease_result:
                            st.markdown("**ðŸ“… Disease Timeline:**")
                            timing = disease_result['timing']
                            if 'onset_days' in timing:
                                st.write(f"â€¢ Onset: {timing['onset_days']} days")
                            if 'peak_period' in timing:
                                st.write(f"â€¢ Peak Period: {timing['peak_period']}")
                            if 'critical_stages' in timing:
                                st.write(f"â€¢ Critical Stages: {', '.join(timing['critical_stages'])}")
                    ...
                    with col2:
                        ...
                        if 'stage_recommendations' in disease_result:
                            st.markdown("**ðŸŒ± Stage-wise Recommendations:**")
...
def show_admin_dashboard():
    ...
    with tab5:
        ...
        st.metric("Total Transaction Value", f"â‚¹{stats['total_transaction_value']:,.2f}")
    ...
    with tab6:
        st.subheader("ðŸ” Create Admin/Agent Account")
        ...
        with st.form("admin_create_account_form"):
            ...
            if st.form_submit_button("Create Account", type="primary"):
                if admin_new_name and admin_new_email and admin_new_password:
                    ...
                    st.error("âŒ Please fill all required fields.")
    
    with tab7:
        st.subheader("ðŸ“Š Market Price Update Logs")
...
def show_farmer_dashboard(selected_model='Enhanced Model'):
    ...
    disease_tab = "ðŸ¦  Disease Prediction"
    ...
    with tab6:
        st.subheader("ðŸ¦  Crop Disease Prediction")
        ...
        if 'last_recommended_crop' in st.session_state:
            ...
            st.info(f"ðŸŒ± Using your last recommended crop: **{recommended_crop.title()}**")
            ...
            st.warning("ðŸš¨ No crop recommendation found. Please get a crop recommendation first from the 'Cultivate' tab.")
            ...
            try:
                ...
                if st.button("ðŸ” Analyze Disease Risk", key="manual_disease_analysis"):
        ...
        if result is not None:
            ...
            if 'predicted_disease' in result and 'disease_prevention' in result:
                ...
                <div style="background: linear-gradient(135deg, {risk_color}, {risk_color}AA); 
                    ...
                    st.markdown("**ðŸ›¡ï¸ Prevention & Management Tips:**")
                    for tip in prevention_tips:
                        st.success(f"âœ… {tip}")
                    
                    if disease_info != 'healthy':
                        st.info("ðŸ’¡ **Important**: Consult with local agricultural experts for specific treatment plans. This prediction is based on environmental conditions and should be used as guidance.")
...
def show_crop_disease_prediction(selected_crop=None, temperature=None, humidity=None):
    ...
    try:
        ...
        st.error(f"âŒ Error loading disease prediction data: {e}")
...
def show_farmer_listings():
    ...
    if listings:
        for listing in listings:
            with st.expander(f"{listing['crop_name'].title()} - {listing['quantity']} kg - â‚¹{listing['expected_price']}/kg"):
                status_text = "Status:"
...
def show_market_price_dashboard():
    ...
    dashboard_title = "ðŸ“Š Market Price Dashboard"
    if current_lang != 'en':
        dashboard_title = translate_text(dashboard_title, current_lang)
    ...
    with col2:
        if st.button("ðŸ”„ Refresh Market Prices", key="refresh_market_prices"):
            ...
            for idx, (_, crop_data) in enumerate(market_prices.iterrows()):
                ...
                with cols[col_idx]:
                    ...
                    <div style="background: white; padding: 15px; border-radius: 10px; 
                        ...
                        <p style="margin: 5px 0; font-size: 18px; font-weight: 600; color: #007bff;">â‚¹{price_per_kg:.2f}/kg</p>
                        <p style="margin: 5px 0; font-size: 14px; color: #666;">â‚¹{price_per_quintal:.0f}/quintal</p>
                        <p style="margin: 5px 0; font-size: 14px; color: {trend_color};">â‡£{trend_icon} {trend}</p>
                    </div>
                    """, unsafe_allow_html=True)
            
            # Add last updated info
            st.markdown("---")
            st.info("ðŸ’¡ Prices are updated regularly. Use this information to make informed decisions about your crops.")
...
def show_agent_dashboard():
    ...
    manage_market_tab = "ðŸ“ Manage Market"
    if current_lang != 'en':
...
def main():
    ...
    <div style="background: linear-gradient(135deg, var(--accent-primary) 0%, var(--accent-secondary) 100%); 
        ...
        <div style="margin-top: 1.5rem; animation: fadeInUp 1.6s ease-out;">
            <span style="display: inline-block; padding: 0.5rem 1rem; 
                        ...
                        animation: bounce 2s infinite;">ðŸš€ AI-Powered</span>
            <span style="display: inline-block; padding: 0.5rem 1rem; 
                        background: rgba(255,255,255,0.2); border-radius: 20px;
                        margin: 0 0.5rem; font-size: 0.9rem; font-weight: 600;
                        animation: bounce 2s infinite 0.2s;">ðŸŒ Multi-Language</span>
            <span style="display: inline-block; padding: 0.5rem 1rem; 
                        background: rgba(255,255,255,0.2); border-radius: 20px;
                        margin: 0 0.5rem; font-size: 0.9rem; font-weight: 600;
                        animation: bounce 2s infinite 0.4s;">ðŸ“± Real-Time</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.sidebar.markdown(f"**ðŸŒ Current Language:** {selected_language}")
    st.sidebar.markdown("---")
    ...
    st.sidebar.markdown(f"**ðŸ§  Selected Model:** {selected_model}")
    st.sidebar.markdown("---")
